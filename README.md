...
## 当直医業務
- 当直医は当直医チェックシートに沿い、時系列でその日の業務を行う。行った仕事はチェックをし、当直の最後に救急ラボ室の封筒に提出する。__やっていないことはチェックしない！__
- 通常業務（平日・休日とも）
...
- 教育回診
    ...
    - 回診中は、パワーハラスメントに該当するため、上級医からの質問は禁ずる (複数の人の前で、わからないことを露呈させる行為)  
    __* 研修中の医師からの、上級医への質問は推奨される__  
    __* 回診以外で、2者しかいない状況では、上級医からの質問は可能である。__
...
## EICU業務
- 担当医:
    - 初期研修医は原則同じ患者、後期研修医以降は、数日で担当患者を変える

- 担当患者業務内容:
    - EICU/CCU入室患者は、循環器内科患者を除き、必ず担当EICU医を設定する
    - 非挿管脳卒中患者のみ、EICU医はバックアップを担当する
    ...
    - 以後は『重症指示書』より指示簿・点滴・内服の指示を出す  
    __* 基本的には指示はセット内の項目を使って指示を出す__
    - 新規指示や処方または変更をした場合は、必ずリーダーNrsに変更を速やかに伝える（電カルのシステム上、すぐに変更点が更新されません）
    - 点滴と処方薬のオーダーは、必ず __次の火曜日__ まで指示を出す
- 指示簿:
    - 指示はセット内の項目を使って指示を出す。適切な項目が見当たらない場合は、「重症病棟指示・指示簿」→「大分類・ICU」より選択する。それでもない場合は、自由記載とする。
...
- 点滴:
  ...
  - 基本的には追加の点滴処方する場合も「セット展開・麻酔科・EICU/GICU・個別注射セット」よりオーダーする
...
- 内服:
    ...
    - 基本的には内服処方も、次の火曜日分まで処方する。短期処方の場合はコメント欄に記載する
    - 処方更新する場合は「つなぎ処方」をする
    - 定期内服薬のスキップ時は、処方カレンダから該当薬剤を選んで右クリックし、「Rp服用中断」を選択。「延長指示」をする事で、中断分のみ延長できる
    - 基本カレンダで火曜日まで内服つながっているはずですが、頓用など含め処方依頼が日中Nrsによりホワイトボードに貼り出されます
    - 内服処方、翌日分の検査は15時までにオーダー。金曜日は週明け月曜日分までオーダー
- 退室時の対応:
    ...
    - 点滴は、病棟マップで患者が転棟先に移動してから、当日の継続分および翌日分を「定時注射のみ」で処方。「重症注射」のままでは点滴が病棟に届きません！
